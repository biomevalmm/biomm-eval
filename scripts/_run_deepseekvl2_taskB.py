...
SEED = 42
MAX_SEQUENCE_CHARS = 8000 #16_000 #100_000
PERTURB_RATE = 0.01
...
{{
  ...
  "sequence_reliance": number between 0 and 1,
  "metadata_reliance": number between 0 and 1,
...
def build_taskB_conversation(row, variant):
    ...
    return conversation, full_len, used_len
...
def call_deepseek_taskB_once(row, variant):
    conversation, full_len, used_len = build_taskB_conversation(row, variant)
    ...
    prepare_inputs = vl_chat_processor(
        ...
        force_batchify=True,
        system_prompt=(
            "You are a biomedical genome classification evaluation assistant. "
...
def call_deepseek_taskB(row, variant, max_retries=MAX_RETRIES):
    last_error = None

    for attempt in range(max_retries):
        try:
            return call_deepseek_taskB_once(row, variant)
...
def main_taskB_deepseekvl2_sequential():
    print_saved_status()
    ...
    for idx, (row_dict, variant) in enumerate(tqdm(todo, total=len(todo)), 1):
        try:
            pred = call_deepseek_taskB(row_dict, variant)
...
main_taskB_deepseekvl2_sequential()
