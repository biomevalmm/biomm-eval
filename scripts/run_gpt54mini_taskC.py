...
# Multiclass ISIC: image + metadata
# Variants:
# full / image_only / metadata_only / metadata_counterfactual
# =========================

import os, re, json, base64, random, time, traceback, argparse, hashlib
...
def image_to_data_url(path: str, max_size: int = 1024, quality: int = 85) -> str:
    img = Image.open(path).convert("RGB")
    img.thumbnail((max_size, max_size))

    tmp = "/tmp/taskC_gpt_tmp.jpg"
...
def call_gpt_taskC(
    ...
    if image_needed:
        content.append({
            "type": "input_image",
            "image_url": image_to_data_url(row["_image_path"], max_size=image_max_size),
        })
        ...
        try:
            response = client.responses.create(
                ...
                text={
                    "format": {
                        "type": "json_schema",
                        "name": "taskC_multiclass_prediction",
                        "schema": {
                            "type": "object",
            ...
            pred = TaskCPrediction.model_validate(obj).model_dump()
            pred = normalize_modalities(pred, variant)
            pred["raw_output"] = response.output_text
            ...
            print(f"[Retry] variant={variant} attempt={attempt}/{max_retries} error={repr(e)} wait={wait}s")
            time.sleep(wait)
...
def main():
    ...
    parser.add_argument(
        "--out_dir",
        type=str,
        default="./outputs/taskC_gpt54mini",
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional. If omitted, uses OPENAI_API_KEY environment variable.",
    )
    parser.add_argument(
        "--run_id",
        type=str,
    ...
    print("\nRunning Task C remaining predictions...")
