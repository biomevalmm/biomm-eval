...
def parse_prediction_response(response_text):
    ...
    used = [m for m in used if m in ["sequence", "metadata"]]

    return {
        "prediction": pred,
        "confidence": safe_float01(obj.get("confidence", 0)),
        "used_modalities": used,
        "sequence_reliance": safe_float01(obj.get("sequence_reliance", 0)),
...
def call_model(row, variant, max_retries=4):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "invalid_request_error" in err or "BadRequestError" in err:
                raise e
