...
RESULTS_JSONL = os.path.join(OUT_DIR, f"taskB_{MODEL_TAG}_results_{RUN_ID}.jsonl")
FAILED_JSONL = os.path.join(OUT_DIR, f"taskB_{MODEL_TAG}_failed_{RUN_ID}.jsonl")
METRICS_CSV = os.path.join(OUT_DIR, f"taskB_{MODEL_TAG}_metrics_{RUN_ID}.csv")
RELIANCE_CSV = os.path.join(OUT_DIR, f"taskB_{MODEL_TAG}_reliance_{RUN_ID}.csv")
REPORT_TXT = os.path.join(OUT_DIR, f"taskB_{MODEL_TAG}_report_{RUN_ID}.txt")

TARGET_COL = "organism_name"
...
def parse_prediction_response(response_text):
    ...
    if start >= 0 and end > start:
        ...
        obj["confidence"] = safe_float01(obj.get("confidence", 0))
        obj["sequence_reliance"] = safe_float01(obj.get("sequence_reliance", 0))
        obj["metadata_reliance"] = safe_float01(obj.get("metadata_reliance", 0))
...
def call_model_once(row, variant):
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            ...
            response_json_schema=TaskBPrediction.model_json_schema(),
            max_output_tokens=700,
        )
    )

    obj = parse_prediction_response(response.text)
...
def load_balanced_data():
    ...
    if len(out) == 0:
        raise RuntimeError("No usable Task B samples found.")
