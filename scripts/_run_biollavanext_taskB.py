...
CONFUSION_DIR = os.path.join(
    OUT_DIR,
    f"confusion_matrices_taskB_{MODEL_TAG}_full_{RUN_ID}"
)
...
def build_taskB_instruction(row, variant):
...
""".strip()
    ...
    return task, full_len, used_len


def format_llava_next_prompt(instruction: str):
    system_msg = (
        "You are a helpful biomedical genome classification evaluation assistant. "
...
# Text-only Task B라 image-token patch는 필수는 아니지만,
# Task C 코드와 호환되게 유지
...
def call_model_taskB_once(row, variant):
    instruction, full_len, used_len = build_taskB_instruction(row, variant)
...
def call_model_taskB(row, variant, max_retries=MAX_RETRIES):
    last_error = None

    for attempt in range(max_retries):
        try:
            return call_model_taskB_once(row, variant)
...
def compute_metrics(results_path, suffix=""):
    ...
    reliance = {
        ...
        "metadata_necessity_score_full_minus_sequence_only": None
        if full is None or seq is None
        else full - seq,
        "sequence_necessity_score_full_minus_metadata_only": None
        if full is None or meta is None
        else full - meta,
        "metadata_counterfactual_sensitivity_full_minus_cf": None
        if full is None or cf is None
        else full - cf,
        "sequence_robustness_drop_full_minus_perturbed": None
        if full is None or pert is None
        else full - pert,
    }
...
def main_taskB_biomed_llava_next():
    print_saved_status()
    ...
    for idx, (row_dict, variant) in enumerate(tqdm(todo, total=len(todo)), 1):
        try:
            pred = call_model_taskB(row_dict, variant)
            ...
            out = {
                ...
                "joined_seq_len": None
                if pd.isna(row_dict.get("joined_seq_len"))
                else int(float(row_dict.get("joined_seq_len"))),
                "host": None
