...
# H100 / Cluster Sequential GPU Version
# ============================================================
...
RESULTS_JSONL = os.path.join(
    OUT_DIR, f"taskC_{MODEL_TAG}_{EVAL_MODE}_results_{RUN_ID}.jsonl"
)
FAILED_JSONL = os.path.join(
    OUT_DIR, f"taskC_{MODEL_TAG}_{EVAL_MODE}_failed_{RUN_ID}.jsonl"
)
METRICS_CSV = os.path.join(
    OUT_DIR, f"taskC_{MODEL_TAG}_{EVAL_MODE}_metrics_{RUN_ID}.csv"
)
RELIANCE_CSV = os.path.join(
    OUT_DIR, f"taskC_{MODEL_TAG}_{EVAL_MODE}_reliance_{RUN_ID}.csv"
)
REPORT_TXT = os.path.join(
    OUT_DIR, f"taskC_{MODEL_TAG}_{EVAL_MODE}_report_{RUN_ID}.txt"
)

CONFUSION_DIR = os.path.join(
    OUT_DIR, f"confusion_matrices_{MODEL_TAG}_{EVAL_MODE}_{RUN_ID}"
)
os.makedirs(CONFUSION_DIR, exist_ok=True)

RESUME = True
INTERIM_EVERY = 50
MAX_NEW_TOKENS = 96#256
DO_SAMPLE = False
TEMPERATURE = 0.0
MAX_RETRIES = 4
IMAGE_MAX_SIZE = 1024
...
def fallback_extract_prediction(response_text: str) -> Dict[str, Any]:
    ...
    # 우선 명시적 약어 찾기
    patterns = [
        ("MEL", r"\bMEL\b|MELANOMA"),
        ("NV", r"\bNV\b|MELANOCYTIC NEVUS|MELANOCYTIC NAEVUS|NEVUS|NAEVUS"),
        ("BCC", r"\bBCC\b|BASAL CELL CARCINOMA"),
        ("BKL", r"\bBKL\b|BENIGN KERATOSIS|KERATOSIS-LIKE|SEBORRHEIC KERATOSIS"),
    ]
    ...
    if hits:
        ...
        raise RuntimeError(f"Could not find prediction class in response: {text[:500]}")
...
def build_taskC_instruction(row, variant):
    if variant == "full":
...
""".strip()
    ...
    return task, include_image, meta_used


def format_llava_next_prompt(instruction: str, include_image: bool):
    system_msg = (
        "You are a helpful biomedical vision-language assistant. "
...
# ============================================================
# Patch LLaVA-NeXT processor/model image-token alignment
# Fix: ValueError("Image features and image tokens do not match")
# ============================================================
...
def call_model_taskC_once(row, variant):
    instruction, include_image, meta_used = build_taskC_instruction(row, variant)
    prompt = format_llava_next_prompt(instruction, include_image)

    if include_image:
        image = load_image(row.get("image_path"))
        inputs = processor(
            images=image,
            text=prompt,
            return_tensors="pt",
...
def call_model_taskC(row, variant, max_retries=MAX_RETRIES):
    last_error = None

    for attempt in range(max_retries):
        try:
            return call_model_taskC_once(row, variant)
...
def compute_metrics(results_path, suffix=""):
    ...
    metrics_path = (
        METRICS_CSV if suffix == "" else METRICS_CSV.replace(".csv", f"{suffix}.csv")
    )
    report_path = (
        REPORT_TXT if suffix == "" else REPORT_TXT.replace(".txt", f"{suffix}.txt")
    )
    reliance_path = (
        RELIANCE_CSV if suffix == "" else RELIANCE_CSV.replace(".csv", f"{suffix}.csv")
    )
    ...
    reliance = {
        ...
        "metadata_necessity_score_full_minus_image_only": None
        if full is None or image_only is None
        else full - image_only,
        "image_necessity_score_full_minus_metadata_only": None
        if full is None or metadata_only is None
        else full - metadata_only,
        "counterfactual_sensitivity_full_minus_cf": None
        if full is None or cf is None
        else full - cf,
    }
...
def main_taskC_biomed_llava_next():
    print_saved_status()
    ...
    for idx, (row_dict, variant) in enumerate(tqdm(todo, total=len(todo)), 1):
        ...
        try:
            pred = call_model_taskC(row_dict, variant)
            ...
            out = {
                ...
                "age_approx_original": None
                if pd.isna(row_dict["age_approx"])
                else float(row_dict["age_approx"]),
                "sex_original": None
                if pd.isna(row_dict["sex"])
                else row_dict["sex"],
                "anatom_site_original": None
                if pd.isna(row_dict["anatom_site_general"])
                else row_dict["anatom_site_general"],
                **pred,
            }
