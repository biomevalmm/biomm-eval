...
# Molmo2-8B Sequential GPU Version
...
print("\n[Load] Loading Molmo2 processor...")
processor = AutoProcessor.from_pretrained(
    MODEL_ID,
    trust_remote_code=True,
)

print("[Load] Loading Molmo2 model...")
...
def call_molmo_taskB_once(row, variant):
...
def call_molmo_taskB(row, variant, max_retries=MAX_RETRIES):
    last_error = None

    for attempt in range(max_retries):
        try:
            return call_molmo_taskB_once(row, variant)
...
def main_taskB_molmo2_sequential():
    print_saved_status()
    ...
    for idx, (row_dict, variant) in enumerate(tqdm(todo, total=len(todo)), 1):
        try:
            pred = call_molmo_taskB(row_dict, variant)
